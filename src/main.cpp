...
int main(void)
{
    ...
    unsigned int sphere1 = engine.createEntity("cube.obj");
    unsigned int sphere2 = engine.createEntity("sphere.stl");
    unsigned int floor = engine.createEntity("floor.stl");

    engine.getEntity(sphere1)->setPosition(vec3(-5.0f, 0.0f, 10.0f));
    engine.getEntity(sphere1)->setScale(1.0f);

    engine.getEntity(sphere2)->setPosition(vec3(5.0f, 0.0f, 10.0f));
